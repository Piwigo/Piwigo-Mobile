...
struct SettingsUtilities
{
    ...
    static func getMailComposer() -> MFMailComposeViewController? {
        ...
        msg += "• defaultThumbnailSize: \((pwgImageSize(rawValue: AlbumVars.shared.defaultThumbnailSize)?.name ?? "?"))\n"
        msg += "• defaultImagePreviewSize: \((pwgImageSize(rawValue: ImageVars.shared.defaultImagePreviewSize)?.name ?? "?"))\n"
