...
public final nonisolated class Tag: NSManagedObject, Identifiable {
    ...
    /**
     Updates a Tag instance with the values from a TagGetInfo.
     */
    public func update(with tagData: TagGetInfo, server: Server) throws {
        ...
        if let newInterval = DateUtilities.timeInterval(from: tagData.lastmodified),
           lastModified != newInterval {
            lastModified = newInterval
        }

        // In the absence of count, use max integer
        if let newCount = tagData.counter, newCount != Int64.max,
