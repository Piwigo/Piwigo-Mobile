...
extension PwgSessionDelegate: URLSessionDataDelegate {
    ...
    public func urlSession(_ session: URLSession, task: URLSessionTask, didReceive challenge: URLAuthenticationChallenge, completionHandler: @escaping (URLSession.AuthChallengeDisposition, URLCredential?) -> Void) {
        ...
        ServerVars.shared.didFailHTTPauthentication = false
        
        // Get HTTP basic authentification credentials
        let service = ServerVars.shared.service
        var account = ServerVars.shared.httpUsername
        ...
        if account.isEmpty || password.isEmpty {
            ...
            ServerVars.shared.httpUsername = account
            KeychainUtilities.setPassword(password, forService: ServerVars.shared.service, account: account)
        }
        ...
        ServerVars.shared.didFailHTTPauthentication = true
        completionHandler(.performDefaultHandling, nil)
    }
}
